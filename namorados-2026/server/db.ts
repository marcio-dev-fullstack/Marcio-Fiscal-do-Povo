...
// Messages queries
export async function createMessage(message: InsertMessage): Promise<void> {
  const db = await getDb();
  if (!db) {
...
export async function getApprovedMessages() {
  ...
  try {
    const result = await db
      .select()
      .from(messages)
      .where(eq(messages.isApproved, 1))
...
export async function getPendingMessages() {
  const db = await getDb();
  if (!db) {
    console.warn("[Database] Cannot get pending messages: database not available");
    return [];
  }
  ...
  try {
    const result = await db
      .select()
      .from(messages)
      .where(eq(messages.isApproved, 0))
      .orderBy(desc(messages.createdAt));
    return result;
  } catch (error) {
    console.error("[Database] Failed to get pending messages:", error);
    return [];
  }
}
...
export async function approveMessage(messageId: number): Promise<void> {
  const db = await getDb();
  if (!db) {
    console.warn("[Database] Cannot approve message: database not available");
    return;
  }
  ...
  try {
    ...
    console.error("[Database] Failed to approve message:", error);
    throw error;
  }
}
